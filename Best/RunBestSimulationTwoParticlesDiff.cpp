                    ...
                    Event* event)
{
  ...
  for (auto& part : conf->tpairs) {
    auto filter = BestAnalysisConfiguration::particleFilter<r>(part, ac);
    if (filter != nullptr) {
  ...
  if (conf->tsingles.size() > 0) {
    ...
    TString singlesTtaskName = TString::Format(conf->taskname.c_str(), "Singles");
...
int main(int argc, char* argv[])
{
  ...
  // =========================
  BestAnalysisConfiguration* conf = nullptr;
  ...
  if (ptRangeLows.size() != ptRangeUps.size() || ptRangeLows.size() != nPtRangeBins.size()) {
    Fatal("RunBestSimulationTwoParticlesDiff", "The limits or the number of bins of the pT ranges differ. Please, fix it!");
  }
  ...
  if (conf->gparticlefilter == "All" && conf->gchargefilter == "All") {
    if (conf->inrapidity) {
      ParticleFilter<AnalysisConfiguration::kRapidity>* particleFilterGen = new ParticleFilter<AnalysisConfiguration::kRapidity>(ParticleFilter<AnalysisConfiguration::kRapidity>::AllSpecies,
                                                                                                                                 ...
                                                                                                                                 genMinPt, genMaxPt,
                                                                                                                                 -abs_y[0], abs_y[0]);
      eventReader = new BestEventReader<AnalysisConfiguration::kRapidity>("BEST", bc, event, eventFilterGen, particleFilterGen);
    } else {
      ParticleFilter<AnalysisConfiguration::kPseudorapidity>* particleFilterGen = new ParticleFilter<AnalysisConfiguration::kPseudorapidity>(ParticleFilter<AnalysisConfiguration::kPseudorapidity>::AllSpecies,
                                                                                                                                             ...
                                                                                                                                             genMinPt, genMaxPt,
                                                                                                                                             -abs_y[0], abs_y[0]);
      eventReader = new BestEventReader<AnalysisConfiguration::kPseudorapidity>("BEST", bc, event, eventFilterGen, particleFilterGen);
    }
  } else {
      ...
      for (auto& ef : conf->teventfilter) {
        ...
        if (conf->inrapidity) {
          if (conf->fillpratt || conf->fillinvmass) {
            if (conf->fillinvmass) {
              if (conf->fillpratt) {
                if (!configureTasks<AnalysisConfiguration::kRapidity, AnalysisConfiguration::kFillPrattAndInvariantMass>(conf, ac, eventFilter, event)) {
                  return 0;
                }
              } else {
                if (!configureTasks<AnalysisConfiguration::kRapidity, AnalysisConfiguration::kFillInvariantMass>(conf, ac, eventFilter, event)) {
                  return 0;
                }
              }
            } else {
              if (!configureTasks<AnalysisConfiguration::kRapidity, AnalysisConfiguration::kFillPratt>(conf, ac, eventFilter, event)) {
                return 0;
              }
            }
          } else {
            if (!configureTasks<AnalysisConfiguration::kRapidity, AnalysisConfiguration::kNoAdditionalOptions>(conf, ac, eventFilter, event)) {
              return 0;
            }
          }
        } else {
          if (conf->fillpratt || conf->fillinvmass) {
            if (conf->fillinvmass) {
              if (conf->fillpratt) {
                if (!configureTasks<AnalysisConfiguration::kPseudorapidity, AnalysisConfiguration::kFillPrattAndInvariantMass>(conf, ac, eventFilter, event)) {
                  return 0;
                }
              } else {
                if (!configureTasks<AnalysisConfiguration::kPseudorapidity, AnalysisConfiguration::kFillInvariantMass>(conf, ac, eventFilter, event)) {
                  return 0;
                }
              }
            } else {
              if (!configureTasks<AnalysisConfiguration::kPseudorapidity, AnalysisConfiguration::kFillPratt>(conf, ac, eventFilter, event)) {
                return 0;
              }
            }
          } else {
            if (!configureTasks<AnalysisConfiguration::kPseudorapidity, AnalysisConfiguration::kNoAdditionalOptions>(conf, ac, eventFilter, event)) {
              return 0;
            }
          }
        }
  ...
  eventReader->reportLevel = repLevel;
  eventLoop->addTask((Task*)eventReader);
  ...
  eventLoop->run(nEventsToRead, nEventsReport);

  TH1::AddDirectory(oldstatus);
  cout << "<INFO> Best event reader analysis - Pair Differential Correlations Histograms  - Completed" << endl;
