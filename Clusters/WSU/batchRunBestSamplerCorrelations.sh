...
HYPERSURFACE=/wsu/home/gm/gm62/gm6269/HYPERSURFACES/C70-80
...
do
  ...
  cmd="sbatch -J batch__BestCorr -q primary --array=1-${NSUBJOBS} --mem-per-cpu=8000 --chdir=${WORKINGDIRECTORY} --time=01:30:00 -o ${WORKINGDIRECTORY}/log/Job_%A_%a.out -e ${WORKINGDIRECTORY}/log/Job_%A_%a.err $CLUSTERMODELWAC/Clusters/WSU/runBestSamplerCorrelations.sh $WORKINGDIRECTORY $HYPERSURFACE $NEVENTSPERJOB $MIXEDEVENTS"
  ...
  do
    ...
    do
      ...
      cmd="sbatch -J $mergeJOBNAME -q primary --chdir=${WORKINGDIRECTORY}/Output --mem-per-cpu=8000 --time=03:00:00 -d afterany:$ARRAYJOBID -o ${WORKINGDIRECTORY}/log/merge/Job_%A.out -e ${WORKINGDIRECTORY}/log/merge/Job_%A.err /wsu/home/gm/gm62/gm6269/PROJECTS/CLUSTERMODELWAC/Clusters/WSU/runMergeResults.sh ${MERGEDFNAME} ${FNAMEPATERN}"
...
do
  ...
  do
    ...
    cmd="sbatch -J waitFinalMerge -q primary --chdir=${BASEDIRECTORY}/${PRODUCTIONDIRECTORY} --mem-per-cpu=8000 --time=03:00:00 -d afterany${MERGEJOBSIDS} -o ${BASEDIRECTORY}/${PRODUCTIONDIRECTORY}/log/merge/SinglesMergeJob_%A.out -e ${BASEDIRECTORY}/${PRODUCTIONDIRECTORY}/log/merge/SinglesMergeJob_%A.err /wsu/home/gm/gm62/gm6269/PROJECTS/CLUSTERMODELWAC/Clusters/WSU/runMergeSubsamples.sh ${MERGEDFNAME}"
...
cmd="sbatch -J waitStatsUncertain -q primary --chdir=${BASEDIRECTORY}/${PRODUCTIONDIRECTORY} --time=01:00:00 -d afterany${MERGEJOBSIDS} -o ${BASEDIRECTORY}/${PRODUCTIONDIRECTORY}/log/merge/WaitStatsUncertainJob.out -e ${BASEDIRECTORY}/${PRODUCTIONDIRECTORY}/log/merge/WaitStatsUncertainJob.err /wsu/home/gm/gm62/gm6269/PROJECTS/CLUSTERMODELWAC/Clusters/WSU/batchRunStatsUncertain.sh best ${BASEDIRECTORY} ${PRODUCTIONDIRECTORY}"
JOBID=($(eval $cmd | tee /dev/tty | awk '{print $4}'))
echo $cmd >> ${BASEDIRECTORY}/${PRODUCTIONDIRECTORY}/log/submit.log
echo "" >> ${BASEDIRECTORY}/${PRODUCTIONDIRECTORY}/log/submit.log
